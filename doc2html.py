#!/usr/bin/env python3
"""
Unified Word/ODT/HTML to clean HTML+CSS converter.
Supports: .doc, .docx, .odt, .html, .htm
...
def choose_file_gui():
    ...
    file_path = filedialog.askopenfilename(
        title="Select Word, ODT, or HTML file",
        filetypes=[
            ("Word / ODT / HTML files", "*.doc *.docx *.odt *.html *.htm"),
            ("All files", "*.*")
        ]
    )
    root.destroy()
...
def convert_to_html(input_path):
    """Convert .doc/.docx/.odt to HTML using LibreOffice. Returns path to generated HTML."""
...
def flatten_text_structure(soup, class_styles):
    ...
    for text_node in soup.find_all(string=True):
        ...
        # Walk up until body, collecting classes and formatting tags
        while current and current.name != 'body':
            ...
            # <font> attributes were already converted to classes in first pass
...
def clean_html(html_path, output_base=None):
    ...
    if output_base is None:
        output_base =  "clean_html_" + html_path.stem
...
def main():
    ...
    if len(sys.argv) >= 2:
        ...
        if USE_GUI:
            ...
            input_file = input("Enter path to Word, ODT, or HTML file: ").strip()
    ...
    if ext in ('.doc', '.docx', '.odt'):
        ...
        print("Unsupported file type. Please provide .doc, .docx, .odt, .html, or .htm")
        sys.exit(1)
